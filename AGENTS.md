...
`bprkazfxqmanrybiexnh`). **VlessHub has its own repo** (`ahmad43ir/VlessHub`) — never add
VlessHub app code, its PWA, its proxy worker, or its bot to this repo.

The **Telegram scraper does NOT live in this repo.** It lives in the dedicated repo
`ahmad43ir/VlessHub` (branch `main`, workflow `scrape.yml`). **Never re-add the scraper or its
workflow to this repo** — that already happened once (Aug 2026) and caused the /scrape dispatch
(422 "No ref found") confusion.
...
- **Logging in / logging out / re-signing-in / re-creating the session** (`sign_in`, new
  ...
  flagged. The session string in the scraper's `.env` (in the VlessHub repo) is
  precious — never regenerate casually.
...
- **Scheduling is currently OFF** — the scraper runs on demand from the Telegram bot via `/scrape`
  (that command dispatches the `vless-scraper` workflow on GitHub Actions in the dedicated
  VlessHub repo — branch `main`; the bot must have the `GH_PAT` secret set, see
  below). Manual runs are naturally rate-safe: at most a handful per day.
...
- Function secret `GH_PAT` (Supabase): fine-grained PAT with **Actions: read & write** on the
  repo. `GH_REPO` defaults to the VlessHub repo (where `scrape.yml` and the scraper's
  repo secrets live), `GH_REF` to `main` — override with function secrets if needed.
...
## 🔄 POST-APPROVAL CHECK (on next session start)

When resuming after the user approved a task (deploy, upload, cleanup, etc.), **always ask**
before doing anything else:

> "Before we continue — did the last task work? Specifically:
> 1. Did the latest deploy (app / function / pages) succeed?
> 2. Were any leftover temp files cleaned up?
> 3. Did the deployed app / bot / page work without errors?"

Wait for the user to confirm or report issues. Do **not** assume success — the user may have
seen errors that weren't captured in the previous session.
