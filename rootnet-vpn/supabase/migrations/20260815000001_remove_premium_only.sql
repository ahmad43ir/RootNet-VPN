-- ============================================================
-- 📁 20260815000001_remove_premium_only.sql
-- ============================================================
-- RootNet v2.2 has NO premium/normal split — every config is public.
-- This migration removes the leftover `premium_only` concept:
--
--   1. Drops the RLS policies on `servers` that reference the column
--      (they must be dropped before the column).
--   2. Drops `premium_only` from `servers` and `vless_links`
--      (the `servers.premium_only` index is dropped automatically).
--   3. Recreates `import_pending_vless_links` without the flag —
--      scraped links become plain public Community servers.
--   4. Recreates the `servers` RLS policy: anon/authenticated read
--      all active servers (no premium filter).
--
-- ⚠️ DESTRUCTIVE-ish: existing `premium_only=true` rows lose their flag
--    (they become public — the intended outcome in v2.2). Run
--      npx supabase db push --dry-run
--    first, then apply after review.
-- ============================================================

-- ──────────────────────────────────────────────
-- 1. Drop policies that reference premium_only
-- ──────────────────────────────────────────────
...
-- 2. Drop the columns
-- ──────────────────────────────────────────────
ALTER TABLE public.servers    DROP COLUMN IF EXISTS premium_only;
ALTER TABLE public.vless_links DROP COLUMN IF EXISTS premium_only;

-- ──────────────────────────────────────────────
-- 3. Import RPC — no premium_only (same parser as
--    20260813000002, flag removed)
-- ──────────────────────────────────────────────
...
-- ──────────────────────────────────────────────
-- 4. Servers RLS — all active servers are public
-- ──────────────────────────────────────────────
