-- ============================================================
-- 📁 20260813000002_propagate_scraped_at_to_servers.sql
-- ============================================================
-- Show the scrape time next to each link in the app.
--
-- The scraper now stamps `vless_links.created_at` with the Telegram
-- message date (when the link was actually scraped/posted). This
-- migration re-creates `import_pending_vless_links` so that timestamp
-- is copied into `servers.created_at` instead of being overwritten
-- with the import time — the app reads `servers.created_at` and
-- displays it next to the config.
--
...
BEGIN
  ...
  FOR v_row IN
    SELECT id, link, premium_only, created_at
    FROM public.vless_links
    WHERE imported_to_servers = false
  ...
  LOOP
    BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            INSERT INTO public.servers
              (name, flag, country, config, host, port, is_active, premium_only, type, config_format, created_at)
            VALUES
              (left(v_name, 64), '🌐', 'Community', v_row.link, left(v_host, 255), v_port, true, v_row.premium_only, v_type, 'sip', v_row.created_at);
      ...
      -- ── Insert as a Community server (scrape time honored) ──
      BEGIN
        INSERT INTO public.servers
          (name, flag, country, config, host, port, is_active, premium_only, type, config_format, created_at)
        VALUES
          (left(v_name, 64), '🌐', 'Community', v_row.link, left(v_host, 255), v_port, true, v_row.premium_only, v_type, 'link', v_row.created_at);
        v_imported := v_imported + 1;
        UPDATE public.vless_links SET imported_to_servers = true WHERE id = v_row.id;
      EXCEPTION WHEN others THEN
        v_skipped := v_skipped + 1;
