...
```
Telegram channel → external scraper (Telethon) → POST /webhook → this worker → Supabase vless_links
                                                                                    ↓
...
- Scraper: external repo — the Telegram listener that calls `/webhook`
