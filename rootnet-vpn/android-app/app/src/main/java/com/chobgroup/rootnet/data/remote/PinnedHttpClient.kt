...
/**
 * Pinned HTTPS client — spec §7.0 / §11.1. Ports the original app's
 ...
 *    SHA-256, primary + backup pin for rotation — spec §11.1).
 *  - **Anti-replay headers** on every request: `X-Request-Timestamp` (epoch
 *    ms) + `X-Request-Id` (unique) — spec §7.0 / §11.3.
...
object PinnedHttpClient {
    ...
    private val certificatePinner = CertificatePinner.Builder()
        .add(
            ...
            // Backup pin — keep old pins during cert rotation (spec §11.1).
    ...
    /** Exponential backoff on retryable statuses + IO errors — spec §7.0. */
