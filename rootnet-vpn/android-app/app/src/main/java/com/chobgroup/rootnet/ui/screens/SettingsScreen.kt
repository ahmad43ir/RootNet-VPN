...
fun SettingsScreen() {
    ...
    Column(
        ...
        Surface(
            ...
            Column {
                ListItem(
                    headlineContent = { Text("More apps", color = RootNetColors.TextPrimary) },
                    supportingContent = { Text("Explore all Chob Group apps", color = RootNetColors.TextSecondary) },
                    leadingContent = { Icon(AppIcons.Apps, contentDescription = null, tint = RootNetColors.AccentNeon) },
                    modifier = Modifier.clickable { ChobGroupLink.open(context) },
                )
                HorizontalDivider(color = RootNetColors.GlassBorder)
                ListItem(
