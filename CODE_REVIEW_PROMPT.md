# RootNet VPN Project — Comprehensive Code Review Prompt

## Project Overview
**RootNet** is a native Android (Kotlin + Jetpack Compose) VPN app using Xray/VLESS protocol, with Supabase backend, Cloudflare Workers, and a Python Telegram scraper for collecting VPN configs.

**Stack:**
- Android: Kotlin, Jetpack Compose, Hilt, Room, Xray-core (via JNI), tun2socks
- Backend: Supabase (PostgreSQL, Edge Functions, Auth, Realtime, Storage)
- Edge Functions: `rootnet-api` (main API), `geo-api` (GeoIP), `telegram-bot` (management bot)
...
- **VPN Engine**: `XrayVpnEngine` wraps Xray-core via JNI, manages TUN interface, config normalization
- **UI**: Compose screens — ServerList, Connect, Settings, Login
- **Auth**: Firebase + Supabase (email/password, premium JWT gating)
- **Monetization**: Unity Ads rewarded video → 30/60 min free sessions; premium = no ads + unlimited
- **Security**: Cert pinning (`PinnedHttpClient`), keystore storage, root detection, anti-replay headers
- **Config**: `UnifiedConfig` normalizes VLESS/VMess/Trojan/SS/WireGuard/SOCKS/SSH/SIP

### 2. Supabase Edge Functions (`supabase/functions/`)
| Function | Purpose |
|----------|---------|
| `rootnet-api` | Main API: `/servers`, `/version`, `/register-device`, `/free-connection`, `/import-vless` (admin) |
| `geo-api` | GeoIP lookup with 24h caching |
| `telegram-bot` | Webhook bot: `/scrape`, `/addproxy`, `/listproxy`, `/addchannel`, server CRUD |

### 3. Cloudflare Workers (`vless-worker/`, `pages-site/`)
| Worker | Purpose |
|--------|---------|
| `vless-worker` | Ingestion webhook: `POST /webhook`, `/webhook/batch`, `/cleanup`, `/health` |
| `pages-site` | Static landing page (`rootnet-proxy`) |

### 4. Python Scraper (`vless-scraper/`)
- **Files**: `main.py`, `proxy_pool.py`, `cleanup_chats.py`
- **Pipeline**: Telegram channels → Telethon → extract VLESS/NPV/SIP → POST to vless-worker → Supabase `vless_links` → pg_cron `import_pending_vless_links()` → `servers` table
- **Proxy Pool**: MTProto proxies from `PROXY_CHANNELS`, tested (cap 3/run), stored in `scraper_proxies`, rotated on failure
- **Limits**: 3 msgs/VLESS channel, 1 msg/proxy channel, 3 proxy tests/run, dead proxies kept 3 days

### 5. Database Schema (Supabase)
Key tables: `servers`, `vless_links`, `scraper_proxies`, `scraper_config`, `app_config`, `device_tokens`, `free_connection_quota`, `rate_limits`, `bot_chat_state`
...
### A. Security & Privacy
1. **No secrets in client** — Verify `AppConstants.kt` has no hardcoded keys; all secrets via Supabase/Worker env
2. **Cert pinning** — `PinnedHttpClient.kt`: pins enforced? Backup pins? Cert rotation handled?
3. **JWT handling** — `SupabaseAuthRepository`: token refresh, expiry, secure storage (Keystore/EncryptedSharedPreferences)?
4. **RLS policies** — All tables have RLS enabled; only `service_role` bypasses; anon keys only read `servers` (premium filtered)
5. **Anti-replay** — `PinnedHttpClient` adds `X-Request-ID` + timestamp; server validates?
...
2. **Xray JSON contract** — `buildXrayConfig` / `buildProxyOutbound`: matches Xray-core expectations? Stream settings (ws/grpc/xhttp/tcp + tls/reality)?
...
4. **Skip dead in test list** — `refresh_pool` filters `is_active=False` (line 459)?
...
4. **Config formats** — `servers.config_format` accepts `link|json|npv|conf|sip` (migration 20260810000001)?
...
3. **Rate limiting** — `rootnet-api`: IP (60/min) + user (30/min) via `checkIpRateLimit` RPC?
...
### G. Build & Deploy
1. **Gradle** — `android-app`: AGP/Kotlin versions current? `compileDebugKotlin` passes?
...
8. `supabase/migrations/20260803000002_add_vless_import_rpc_and_cron.sql`
