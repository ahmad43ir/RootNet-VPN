 ...
 * Xray engine JSON (the engine is gone) — the parsers are kept for live TCP
...
object ConfigNormalizer {
    ...
    fun detectFormat(raw: String): String {
        ...
        // Base64-encoded JSON (VMess) — decoded payload starts with '{'
        try {
            val decoded = String(Base64.getDecoder().decode(trimmed), StandardCharsets.UTF_8)
    ...
    fun fromVmessLink(raw: String): UnifiedConfig {
        ...
        // Some generators emit vmess:///base64 (extra slash) — strip exactly one.
     ...
     * Never re-enters [fromNpv] — recursion is bounded by a depth guard so a
    ...
    private fun resolveNpvNode(value: Any?, protocolHint: String?, depth: Int = 0): UnifiedConfig? {
        ...
        return when (value) {
            ...
            is String -> {
                ...
                if (!s.contains("://")) {
                    // Likely a base64 VMess payload — decode then re-resolve.
                    try {
                        val decoded = String(Base64.getDecoder().decode(s), StandardCharsets.UTF_8)
                            .trimStart()
                        if (decoded.startsWith("{")) {
                            return resolveNpvNode(JSONObject(decoded), protocolHint, depth + 1)
                        }
                    } catch (_: Exception) {
                        // not base64 — fall through to link parsing
                    }
                }
    ...
    // ─── SIP format (SocksIP / SSH / SOCKS5 / HTTP proxy configs) ──────────
