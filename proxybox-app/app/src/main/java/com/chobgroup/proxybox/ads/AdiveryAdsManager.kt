 ...
 * RootNet v2.3).
 *
 *  - **Interstitial** (picture ad) → every **3rd Copy/Share/Open** tap on a
...
object AdiveryAdsManager {
    ...
    // ── CONFIGURATION — Adivery dashboard (ProxyBox app) ───────────────────
    // App code `0d0ce797-932b-4190-8b1a-79e1b504786d` with three placements:
    // image (interstitial) d5738d1f-…, video (rewarded) af125f4e-….
    // The banner placement is not wired yet — leave the placeholder so the
    ...
    // One picture (interstitial) ad per 60s max — app-wide (mirrors RootNet).
    ...
    // ── Interstitial (3rd-distinct-tap gate, mirrors RootNet) ──────────────
     ...
     * on screen — its pending callback fires on close).
     */
